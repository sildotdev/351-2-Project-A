...
//   g_canvas.onmouseup = 		function(ev){myMouseUp(  ev) };
  ...
  g_grid.init()  
  // g_partA.initBouncy2D(4);          // create a 2D bouncy-ball system where
...
// you don't need to respond separately to key-down and key-up events.
  ...
  g_timeStepMax = g_timeStep;
	...
	// in the <div> element with id='KeyResult':r 

//   document.getElementById('KeyResult0').innerHTML = 
//    			'char= ' 		 	+ myChar 			+ ', keyCode= '+ ev.keyCode 	+ 
//    			', charCode= '+ ev.charCode + ', shift= '	 + ev.shiftKey 	+ 
//    			', ctrl= '		+ ev.shiftKey + ', altKey= ' + ev.altKey 		+ 
//    			', metaKey= '	+ ev.metaKey 	+ '<br><br>' ;
  ...
  // update particle system state? g_partA.runMode 0=reset; 1= pause; 2=step; 3=run
	switch(myChar) {
		...
		case 'B':
			if(g_partA.bounceType==0) g_partA.bounceType = 1;
		...
		case 'g':			// REDUCE gravity
			g_partA.grav *= 0.99;		// shrink 1%
			break;
		case 'G':
			g_partA.grav *= 1.0/0.98;	// grow by 2%
			break;
		case 'm':
		...
		case 'R':  // HARD reset: position AND velocity.
		  g_partA.runMode = 0;			// RESET!
			g_partA.s1[PART_XPOS] =  0.0;	
			g_partA.s1[PART_YPOS] =  0.0;	
      ...
      g_partA.s1[PART_ZVEL] =  0.0;
		...
		case 'r':		// 'refresh' -- boost velocity only; return to 'run'
		  g_partA.runMode = 3;  // RUN!
			if(g_partA.s1[PART_XVEL] > 0.0) g_partA.s1[PART_XVEL] +=       g_partA.INIT_VEL; 
			else g_partA.s1[PART_XVEL] -=       g_partA.INIT_VEL;
			if(g_partA.s1[PART_YVEL] > 0.0) g_partA.s1[PART_YVEL] += 0.9 * g_partA.INIT_VEL; 
			else g_partA.s1[PART_YVEL] -= 0.9 * g_partA.INIT_VEL;
			break;	
		case 's':
		case 'S':
			// switch to a different solver:
			if(g_partA.solvType == 0) g_partA.solvType = 1;
			else g_partA.solvType = 0;
			break;
		case 'p':
