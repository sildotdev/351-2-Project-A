...
// (JT: why the numbers? counts columns, helps meF keep 80-char-wide listings)
//
// ORIGINAL SOURCE:
...
var g_partC = new PartSys(); // create our first particle-system object;
var g_colGridC = new CollGrid();

var g_partD = new PartSys(); // create our first particle-system object;
var g_colGridD = new CollGrid();
...
function main() {
  ...
  g_partB.init()
  g_partB.initFlocking(100);
  g_colGridB.init()

  g_partC.init()
  g_partC.initFireReeves(600);
  g_colGridC.init()

  g_partD.init()
  g_partD.initSpringPair(10);
  g_colGridD.init()
...
function resetPos() {
  ...
  for(var i = 0; i < g_partA.partCount; i += 1, j+= PART_MAXVAR) {
    ...
    if(  g_partA.s2[j + PART_XVEL] > 0.0) // ADD to positive velocity, and 
         g_partA.s2[j + PART_XVEL] += 1.7 + 4.0*g_partA.randX*g_partA.INIT_VEL;
                                          // SUBTRACT from negative velocity: 
    else g_partA.s2[j + PART_XVEL] -= 1.7 + 4.0*g_partA.randX*g_partA.INIT_VEL; 

    if(  g_partA.s2[j + PART_YVEL] > 0.0) 
         g_partA.s2[j + PART_YVEL] += 1.7 + 4.0*g_partA.randY*g_partA.INIT_VEL; 
    else g_partA.s2[j + PART_YVEL] -= 1.7 + 4.0*g_partA.randY*g_partA.INIT_VEL;

    if(  g_partA.s2[j + PART_ZVEL] > 0.0) 
         g_partA.s2[j + PART_ZVEL] += 1.7 + 4.0*g_partA.randZ*g_partA.INIT_VEL; 
    else g_partA.s2[j + PART_ZVEL] -= 1.7 + 4.0*g_partA.randZ*g_partA.INIT_VEL;
    }

    var j=0; // array index for particle i
    for(var i = 0; i < g_partB.partCount; i += 1, j+= PART_MAXVAR) {
      g_partB.roundRand();  // make a spherical random var.
      if(  g_partB.s2[j + PART_XVEL] > 0.0) // ADD to positive velocity, and 
           g_partB.s2[j + PART_XVEL] += 1.7 + 4.0*g_partB.randX*g_partB.INIT_VEL;
                                            // SUBTRACT from negative velocity: 
      else g_partB.s2[j + PART_XVEL] -= 1.7 + 4.0*g_partB.randX*g_partB.INIT_VEL; 

      if(  g_partB.s2[j + PART_YVEL] > 0.0) 
           g_partB.s2[j + PART_YVEL] += 1.7 + 4.0*g_partB.randY*g_partB.INIT_VEL; 
      else g_partB.s2[j + PART_YVEL] -= 1.7 + 4.0*g_partB.randY*g_partB.INIT_VEL;

      if(  g_partB.s2[j + PART_ZVEL] > 0.0) 
           g_partB.s2[j + PART_ZVEL] += 1.7 + 4.0*g_partB.randZ*g_partB.INIT_VEL; 
      else g_partB.s2[j + PART_ZVEL] -= 1.7 + 4.0*g_partB.randZ*g_partB.INIT_VEL;
      }

      var j=0; // array index for particle i
      for(var i = 0; i < g_partC.partCount; i += 1, j+= PART_MAXVAR) {
        g_partC.roundRand();  // make a spherical random var.
        if(  g_partC.s2[j + PART_XVEL] > 0.0) // ADD to positive velocity, and 
             g_partC.s2[j + PART_XVEL] += 1.7 + 4.0*g_partC.randX*g_partC.INIT_VEL;
                                              // SUBTRACT from negative velocity: 
        else g_partC.s2[j + PART_XVEL] -= 1.7 + 4.0*g_partC.randX*g_partC.INIT_VEL; 

        if(  g_partC.s2[j + PART_YVEL] > 0.0) 
             g_partC.s2[j + PART_YVEL] += 1.7 + 4.0*g_partC.randY*g_partC.INIT_VEL; 
        else g_partC.s2[j + PART_YVEL] -= 1.7 + 4.0*g_partC.randY*g_partC.INIT_VEL;

        if(  g_partC.s2[j + PART_ZVEL] > 0.0) 
             g_partC.s2[j + PART_ZVEL] += 1.7 + 4.0*g_partC.randZ*g_partC.INIT_VEL; 
        else g_partC.s2[j + PART_ZVEL] -= 1.7 + 4.0*g_partC.randZ*g_partC.INIT_VEL;
        }
...
function onSphere() {
  g_partA.circle = !g_partA.circle;
}

var g_solver = false;
var firstTime = true;
function updateSolver() {
  solverID = document.getElementById("select1");
  ...
  if (firstTime) {
    ...
    // select random solver
    g_solver = Math.floor(Math.random() * allSolvers.length);
    g_solver = g_solver.toString();

    // set solverID to corresponding value
    solverID.value = g_solver;

    firstTime = false;
    return;
  } else {
    g_solver = solverID.value;
  }

  console.log(g_solver)

  vbos = [g_partA, g_partB, g_partC, g_partD];
  ...
  switch (g_solver) {
    case "0":
      ...
      break;
    case "1":
      g_solver = SOLV_OLDGOOD;
      break;
    case "2":
      g_solver = SOLV_MIDPOINT;
      break;
    case "3":
      g_solver = SOLV_RUNGEKUTTA;
      break;
    case "4":
      g_solver = SOLV_BACK_EULER;
      break;
    case "5":
      g_solver = SOLV_BACK_MIDPT;
      break;
    case "6":
      g_solver = SOLV_BACK_ADBASH;
      break;
    case "7":
      g_solver = SOLV_VERLET;
      break;
    case "8":
      g_solver = SOLV_VEL_VERLET;
      break;
    case "9":
      g_solver = SOLV_LEAPFROG;
      break;
    default:
      break;
  }
...
function draw() {
  ...
  if (g_partC.runMode > 1) {
    if (g_partC.runMode == 2) g_partC.runMode = 1;

    g_partC.solver();
    g_partC.doConstraints();
    g_partC.step();
  ...
  if (g_partD.runMode > 1) {
    if (g_partD.runMode == 2) g_partD.runMode = 1;

    g_partD.solver();
    g_partD.doConstraints();
    g_partD.step();
  }
...
var g_Camera = new Vector3([40.0, 0.0, 2.0]);
